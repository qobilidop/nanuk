...
def assert_map_matches(name, golden, rtl):
    ...
    assert tuple(rtl.md) == tuple(golden.md), f"{name}: md"
...
@pytest.mark.parametrize(
    "map_key,tables",
    [("l2fwd", L2_TABLES), ("ttl", L2_TABLES), ("push", PUSH_TABLES)],
)
def test_demo_programs_cosim(progs, map_key, tables):
    for name, pkt in map_packets():
        for ingress in (0, 1):
            pp = run_program(progs["pp"], pkt, [ingress])
            if pp.verdict != VERDICT_ACCEPT:
                continue
            golden = run_map(progs[map_key], pkt, pp, tables, pp.md)
            rtl = run_map_one(progs[map_key], pkt, pp, tables, pp.md)
...
def test_tunnel_pop_cosim(progs):
    ...
    pp1 = run_program(progs["pp"], inner, [0])
    pushed = run_map(progs["push"], inner, pp1, PUSH_TABLES, pp1.md)
    assert pushed.sent and pushed.delta == 22
    for name, frame in [("tunnel", pushed.frame), ("plain", inner)]:
        pp2 = run_program(progs["pp_tunnel"], frame, [1])
        assert pp2.verdict == VERDICT_ACCEPT
        golden = run_map(progs["pop"], frame, pp2, POP_TABLES, pp2.md)
        rtl = run_map_one(progs["pop"], frame, pp2, POP_TABLES, pp2.md)
        assert_map_matches(f"pop/{name}", golden, rtl)


def test_composed_pipeline_rtl_vs_golden(progs):
    for name, pkt in map_packets():
        gp, gm = run_pipeline(progs["pp"], progs["l2fwd"], pkt, L2_TABLES, [1])
        rp, rm = run_pipeline_rtl(progs["pp"], progs["l2fwd"], pkt, L2_TABLES, [1])
