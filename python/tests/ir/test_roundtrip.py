    ...
    return ir.Program(
        ...
        states=[
            ir.State(
                name="start",
                ops=[
                    ir.Op(mark=ir.Mark(hdr_id=0, emit_sethdr=True, debug_name="outer")),
                    ...
                    ir.Op(advance=ir.Advance(const_bytes=7)),
                    ir.Op(advance=ir.Advance(value_id=3)),
                    ir.Op(mark=ir.Mark(emit_sethdr=False, debug_name="inner")),
                ],
            ...
            ir.State(
                name="take",
                ops=[ir.Op(mark=ir.Mark(hdr_id=1, emit_sethdr=True, debug_name="inner"))],
                terminator=ir.Terminator(goto=ir.Goto(target_state="fin")),
            ),
            ir.State(name="fin", terminator=ir.Terminator(halt=ir.Halt(drop=False))),
        ],
    )
