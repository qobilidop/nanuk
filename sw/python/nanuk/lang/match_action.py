...
The metadata window (md): 8 x 16-bit slots flowing through the core like
the frame does — the system loads them, the PP may read/write them, the
MAP reads/writes them, and the final state is the core's metadata output.
Slot meanings are system conventions (nanuk_switch: slot 0 = ingress in /
egress bitmap out; slots 4-7 program-pair private).
"""
...
class MatchActionStateCompiler:
    ...
    def load_md(self, slot: int) -> MatchActionValue:
        """Load a metadata-window slot (LDMD)."""
        self._check_open()
        if not isinstance(slot, int) or not 0 <= slot < _MD_SLOTS:
            raise CompileError(f"load_md slot {slot!r} out of range 0..{_MD_SLOTS - 1}")
        name = f"md[{slot}]"
        v = MatchActionValue(next(self._value_ids), name)
        self._ops.append(
            ir.MatchActionOp(load_md=ir.MdLoad(value_id=v.value_id, slot=slot, debug_name=name))
        )
        return v

    def and_imm(self, value: MatchActionValue, imm: int) -> MatchActionValue:
        """value & zero-extended 16-bit immediate (ANDI)."""
        self._check_open()
        self._require_value(value, "and_imm")
        if not isinstance(imm, int) or not 0 <= imm <= _MAX_IMM16:
            raise CompileError(f"and_imm immediate {imm!r} out of range 0..{_MAX_IMM16:#x}")
    ...
    def shift(self, value: MatchActionValue, amount: int) -> MatchActionValue:
        """value << amount, truncating at 64 bits (SHLI)."""
        self._check_open()
        self._require_value(value, "shift")
        if not isinstance(amount, int) or not 0 <= amount <= 63:
            raise CompileError(f"shift amount {amount!r} out of range 0..63")
        v = MatchActionValue(next(self._value_ids), f"{value.name} << {amount}")
        self._ops.append(
            ir.MatchActionOp(shift=ir.Shift(value_id=v.value_id, src_value_id=value.value_id, amount=amount))
        )
        return v
    ...
    def store_md(self, value: MatchActionValue, slot: int, *, nunits: int = 1) -> None:
        """Store a value into metadata-window slots (STMD, MSB-first)."""
        self._check_open()
        self._require_value(value, "store_md")
        if not isinstance(slot, int) or not 0 <= slot < _MD_SLOTS:
            raise CompileError(f"store_md slot {slot!r} out of range 0..{_MD_SLOTS - 1}")
        if not 1 <= nunits <= 4 or slot + nunits > _MD_SLOTS:
            raise CompileError(
                f"store_md nunits {nunits!r} invalid for slot {slot} (1..4, in-window)"
            )
        self._ops.append(
            ir.MatchActionOp(store_md=ir.MdStore(value_id=value.value_id, slot=slot, nunits=nunits))
        )

    def csum(self, length: MatchActionValue, field=None, *, hdr=None,
             byte_offset: int | None = None) -> MatchActionValue:
        """Ones-complement checksum (CSUM) of window bytes starting at a
        bound header (or raw hdr/byte_offset addressing), over a runtime
        length value. Protocol-agnostic: the program zeroes and stores the
        checksum field itself with store()."""
        self._check_open()
        ...
        if isinstance(field, BoundHeader):
            ...
            raise CompileError("csum expects a bound header or hdr=/byte_offset=")
        if not _MIN_OFF <= off <= _MAX_OFF:
            raise CompileError(f"csum byte offset {off} out of range")
        v = MatchActionValue(next(self._value_ids), name)
        self._ops.append(
            ir.MatchActionOp(
                csum=ir.Csum(value_id=v.value_id, hdr_id=hdr_id, byte_offset=off,
                             len_value_id=length.value_id)
            )
        )
        return v
    ...
    def send(self, *, delta: int = 0, egress: MatchActionValue | None = None) -> None:
        """Terminate: transmit with a signed head delta.

        egress= is system-convention sugar: it stores the value to md
        slot 0 (the nanuk_switch egress-bitmap slot) before sending."""
        self._check_open()
