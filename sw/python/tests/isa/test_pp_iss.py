...
from nanuk.isa.pp_asm import assemble, assemble_with_lines
from nanuk.isa.pp_iss import (
    ERR_HDR_VIOLATION,
    ERR_ILLEGAL,
    ERR_PC_RANGE,
    ERR_MD_RANGE,
...
def test_movi_advr_halt():
    ...
    r = run_pp_iss(binary, b"\x00" * 64, line_map=lines)
...
def test_reserved_bits_and_bad_reg_are_illegal():
    r = run_pp_iss(raw(pp_encoding.encode_movi("r0", 1) | (1 << 22)), b"")
    assert r.error == ERR_ILLEGAL
    r = run_pp_iss(raw((0x04 << 26) | (5 << 23)), b"")  # reg code 5
    assert r.error == ERR_ILLEGAL
...
def test_rz_reads_zero_discards_writes():
    src = "    movi rz, 7\n    advr rz\n    halt accept\n"
    r = run_pp_iss(assemble(src), b"\x00" * 8)
    assert r.payload_offset == 0


def test_trace_snapshots_hdr():
    src = "    advi 14\n    sethdr 3\n    halt accept\n"
    r = run_pp_iss(assemble(src), b"\x00" * 64)
    step = r.trace[1]
