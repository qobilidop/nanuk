...
from nanuk.testkit.testkit import DMAC, NO_TABLE, demo_flood_table, demo_tun_table
...
PUSH_TABLES = [NO_TABLE, TUN_TABLE, NO_TABLE, demo_flood_table()]
...
def test_push_prepends_exact_outer_header(progs):
    ...
    pp, mp = run_pipeline(
        progs["pp_l2l3l4"], progs["map_push"], inner, PUSH_TABLES, [0]
    )
    assert pp.accepted
    assert mp is not None and mp.sent
    assert mp.md[0] == 0x2 and mp.delta == 22
    assert len(OUTER) == 22
...
def test_push_pop_round_trip(progs):
    ...
    pp2, pop = run_pipeline(
        progs["pp_tunnel"], progs["map_pop"], push.frame, POP_TABLES, [1]
    )
    assert pp2.accepted
    assert pp2.md[5] == 0x4E4B, "PP flags the tunnel via md slot 5"
...
def test_non_tunnel_traffic_floods_unchanged_through_pop(progs):
    ...
    pp, pop = run_pipeline(progs["pp_tunnel"], progs["map_pop"], plain, POP_TABLES, [2])
    assert pp.accepted
    assert pp.md[5] == 0
    assert pop is not None and pop.sent
    assert pop.delta == 0
    assert pop.frame == plain
    assert pop.md[0] == 0xB  # flood, ingress 2 excluded
...
def test_bad_magic_drops_at_pp(progs):
    ...
    pp, pop = run_pipeline(progs["pp_tunnel"], progs["map_pop"], bytes(frame), POP_TABLES, [0])
...
def test_unknown_dmac_bypasses_tunnel(progs):
    ...
    pp, mp = run_pipeline(
        progs["pp_l2l3l4"], progs["map_push"], inner, PUSH_TABLES, [3]
    )
    assert mp is not None and mp.sent
    assert mp.delta == 0
