"""SIIT translator, match-action side (MAP), nanuk-lang edition. The
hand-written ISA copy is translate.asm next to this file; this twin is
behaviorally identical (verdict / error / md / output frame) over every
committed conformance vector, and pairs with parse.py.

RFC 7915 stateless IP/ICMP translation on the Nanuk core: IPv4 <-> IPv6 header
rewrite in place, RFC 7757 EAMT (exact-match tables t0/t1/t2) with RFC 6052
pool6 (64:ff9b::/96, baked as immediates) as the fallback, RFC 1624
incremental L4 checksum patching, and the value half of the ingress ledger (the
PP already refused everything structural). Every drop is an explicit DROP.
...
Two nanuk-lang idioms carry the arithmetic the raw ISA spells by hand:
  ...
  * The ISA has no register-register compare, only compare-to-immediate, so a
    ...
    against zero, exactly as the asm does.
  * The RFC 1624 end-around-carry fold is done by writing the wide accumulator
    ...
    byte range, which is precisely HC' = ~fold(~HC + ~m + m'). This is the same
    "the byte machine folds for us" move the asm makes with CSUM over the
    address ranges — here applied to a spilled accumulator. Scratch lives in
    the 32-byte headroom below the outgoing frame (dead src/dst staging by the
    time the patch runs), so it never reaches the wire.
...
md[5] = IHL bytes (v4 arm); md[6] = TOS (v4 arm). md[0] (system egress) is
never written — the translator is a middlebox function, not a forwarder.

Standalone document: every constant is inline; nothing is imported from sibling
examples.
"""
